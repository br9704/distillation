...
2. Disjointness is **asserted on the id**, not assumed — in `split.py` at split time, and
...
student is trained and evaluated on the user turn **only** — no system block:
...
Measured, not estimated — `src/measure_tokens.py` → `results/token_counts.json`.
